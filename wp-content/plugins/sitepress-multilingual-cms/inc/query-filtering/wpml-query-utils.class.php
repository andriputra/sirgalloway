...
class WPML_Query_Utils extends WPML_WPDB_User {
	 ...
	 */
	public function date_query_has_posts( $lang, $year, $month = null, $day = null, $post_type = 'post' ) {
		$post_types        = (array) $post_type;
		$post_type_snippet = (bool) $post_types ? " AND post_type IN (" . wpml_prepare_in( $post_types ) . ") " : "";
		$month_snippet     = (bool) $month === true ? $this->wpdb->prepare( ' AND month(p.post_date) = %d ',
		                                                                    $month ) : '';
		...
		return $this->wpdb->get_var( $this->wpdb->prepare( "
						...
						WHERE year(p.post_date) = %d
							" . $month_snippet . $day_snippet . $post_type_snippet . "
						  AND post_status='publish'
						  AND language_code = %s",
		                                                   $year,
		                                                   $lang ) );
