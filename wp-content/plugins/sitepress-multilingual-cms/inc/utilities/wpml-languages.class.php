...
class WPML_Languages extends WPML_SP_And_PT_User {
	...
	public function add_date_or_cpt_url_to_ls_lang( $lang, $current_query, $icl_lso_link_empty, $skip_lang ) {
		...
		$mark_missing = false;
		$override     = false;
		if ( $current_query->is_year() && $this->query_utils->date_query_has_posts( $lang_code,
		                                                                            $year,
		                                                                            null,
		                                                                            null,
		                                                                            $post_type )
		...
		} elseif ( $current_query->is_month() && $this->query_utils->date_query_has_posts( $lang_code,
		                                                                                   $year,
		                                                                                   $month,
		                                                                                   null,
		                                                                                   $post_type )
		...
		} elseif ( $current_query->is_day() && $this->query_utils->date_query_has_posts( $lang_code,
		                                                                                 $year,
		                                                                                 $month,
		                                                                                 $day,
		                                                                                 $post_type )
			...
			if ( $this->sitepress->is_translated_post_type( $post_type ) ) {
				$url                    = $this->sitepress->convert_url( $this->wp_api->get_post_type_archive_link( $post_type ),
				                                                         $lang_code );
